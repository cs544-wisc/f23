...
def get_cell_output(notebook, expected_cell_txt, print_output = False):
    # Get the cell idx
    ...
    if print_output:
        print("Cell output of", cell_output, "for", cell_idx)
    ...
    for output in cell_output:
        if (output["output_type"] == "stream" and output["name"] == "stdout"):
            ...
            std_out += output["data"]["text/plain"]
        elif output["output_type"] == "error":
            err_out += output["evalue"]
            
    return std_out.strip(), err_out.strip()

def cell_has_text(notebook, cell_search_txt, expected_txt, print_output = False):
    cell_search_txt = cell_search_txt.strip()
    expected_txt = expected_txt.strip()

    stdout, error = get_cell_output(notebook, cell_search_txt, print_output = print_output)
    if stdout is None or error is None:
        return "Have no output for cell with text " + cell_search_txt

    if len(error) > 0:
        return "Cell with text " + cell_search_txt + " failed due to error: " + error
...
def verify_files_present():
    ...
    if len(missing_comments) > 0:
        raise Exception(f"Couldn't find cells with comment(s) {missing_comments} in {notebook_file}")
...
def init_runner(test_dir):
    ...
    subprocess.check_output(["docker", "build", ".", "-t", "p6-base"], env = environment)
    ...
    subprocess.call(["cp", "docker_autograde.py", "nb/docker_autograde.py"], env = environment)
    ...
    result = subprocess.run("docker kill p6-db-2", capture_output = True, text = True, shell = True, env = environment)
    time.sleep(10)
...
def init(*args, **kwargs):  
    ...
    if not os.path.exists(expected_path):
        raise Exception(f"Couldn't find results file at {expected_path}. Check the out files in {output_dir} for details")
...
def q6():
    ...
    expected_terms = ["stations", "temporary"]
    for expected_txt in output:
        if expected_txt not in output:
            return f"couldn't find txt {expected_txt} in output {output}"
...
def q7():
    ...
    dict_words = output_line[ output_line.index("{") : output_line.index("}") + 1 ]
